...
/**
 * ErrorPage404 — glitch-text 404 with a crying bear that cheers up on hover.
 *
 * Converted from a CodePen-style snippet (window.React/ReactDOM/styled
 * globals + a separate CSS panel using pseudo-Sass "%s" interpolation) into
 * a normal, importable React component with no external CSS file and no
 * styled-components dependency:
 *   - The "'hsl(...)' % var(...)" string-interpolation hacks are rewritten
 *     as plain CSS: modern hsl()/hsla() accept calc() and var() directly.
 *   - styled.span`` with an empty template was just a plain <span>; those
 *     are now regular elements styled by class name in the embedded <style>.
 *   - ReactDOM.render(...) is removed — this is a component you mount
 *     wherever you handle a 404 (e.g. a React Router "*" route), not a
 *     script that renders itself.
 *   - The devicemotion listener is now added/removed with
 *     addEventListener/removeEventListener instead of overwriting
 *     window.ondevicemotion, so it cleans up on unmount.
 *
 * Visual behavior is otherwise unchanged: moving the mouse (or tilting on
 * mobile) parallaxes the bear logo and individual glitching characters in
 * "404" / "Not found", each by its own factor. Hovering "Return to
 * happiness" stops the bear's tears and opens its eyes.
 *
 * IMPORTANT: the exact strings "404" and "Not found" are load-bearing —
 * the parallax CSS targets specific nth-of-type positions of the rendered
 * characters. If you change that copy, update the corresponding
 * `.code-text span:nth-of-type(n)` / `.code-message-text span:nth-of-type(n)`
 * rules below to match the new character positions.
 */

const GLITCH_CHARS = "`¡™£¢∞§¶•ªº–≠åß∂ƒ©˙∆˚¬…æ≈ç√∫˜µ≤≥÷/?░▒▓<>/".split("");

function GlitchyText({ children, className = "" }) {
  const chars = useMemo(() => {
    return children.split("").map((char) => {
      const style = { "--count": Math.random() * 5 + 1 };
      for (let i = 0; i < 10; i++) {
        style[`--char-${i}`] = `"${
  ...
  return (
    <h1 className={`glitchy-text ${className}`}>
      <span className="glitchy-text__char--readable">{children}</span>
      {chars.map(({ char, style }, idx) => (
        <span
          key={idx}
...
function BearLogo({ className = "" }) {
  return (
    <svg
      ...
      role="img"
      aria-label="A bear wearing sunglasses, crying"
    >
      <g transform="matrix(1.34105 0 0 1.34105 -51.157 -1049.694)">
        ...
        />
        <circle className="bear__eye bear__eye--left" r="11.185" cy="894.081" cx="97.242" />
        <circle className="bear__eye bear__eye--right" r="11.185" cx="201.851" cy="894.081" />
    ...
    <div className="nf404">
      <style>{`
        ...
        @media (min-width: 768px) {
          .nf404 .code-text { font-size: 6rem; }
        }

        .nf404 .code-message-text { font-size: 1.5rem; }
        @media (min-width: 768px) {
          .nf404 .code-message-text { font-size: 3rem; }
        }
        ...
        .nf404 .glitchy-text__char--readable {
          opacity: 0;
          position: absolute;
        }
        @keyframes nf404-glitch-switch {
        ...
        .nf404 .glitchy-text__char {
          color: transparent;
          position: relative;
        }
        .nf404 .glitchy-text__char:after {
          --txt: attr(data-char);
        ...
        /* Bear */
        .nf404 .bear__shades { opacity: var(--bear-shades); }
        .nf404 .bear__tear-stream { fill: hsla(200, 100%, 75%, var(--bear-tears, 0)); }
        .nf404 .bear__brows { stroke: hsla(0, 0%, 0%, var(--bear-brows, 0)); }
        .nf404 .bear__eye { fill: hsla(0, 0%, 0%, var(--bear-eyes, 0)); }
        .nf404 .bear-logo {
          grid-row: span 2;
          position: relative;
          max-width: 400px;
        ...
        .nf404 .bear__eye--teary { fill: hsla(200, 100%, 75%, calc(var(--bear-tears, 0) - 0.5)); }
        @media (min-width: 768px) {
          .nf404 .bear-logo { height: 12rem; width: 12rem; }
        }

        .nf404 .return-link:hover ~ .bear-logo--tears {
          --bear-brows: 0;
          --bear-tears: 0;
        }
        .nf404 .bear-logo--tears {
          --bear-shades: 0;
          --bear-brows: 1;
          --bear-tears: 0.75;
          --bear-eyes: 1;
        }
        .nf404 .bear-logo--tears path,
        .nf404 .bear-logo--tears circle,
        .nf404 .bear-logo--tears ellipse {
          transition: fill .2s ease 0s, transform .2s ease 0s;
        }
        .nf404 .return-link:hover ~ .bear-logo--tears .bear__eye {
          transform: scaleY(1);
          fill: hsla(0, 100%, 0%, var(--bear-eyes));
        }
        .nf404 .bear-logo--tears .bear__eye {
          transform: scaleY(0.25);
          transform-origin: 97px 894px;
          fill: hsla(200, 100%, 75%, var(--bear-eyes));
        }
        .nf404 .bear-logo--tears .bear__eye--right { transform-origin: 201px 894px; }

        /* Mousemove / device-motion parallax — tied to exact "404" and
           "Not found" character positions (see file header note). */
        .nf404 .bear-logo--tears { --scaleX: -50; --scaleY: -50; }
        .nf404 .code-text span:nth-of-type(2) { --scaleX: 30; --scaleY: -20; }
        .nf404 .code-text span:nth-of-type(3) { --scaleX: -20; --scaleY: 50; }
        .nf404 .code-text span:nth-of-type(4) { --scaleX: -30; --scaleY: 40; }
        .nf404 .code-message-text span:nth-of-type(2) { --scaleX: 20; --scaleY: 10; }
        .nf404 .code-message-text span:nth-of-type(3) { --scaleX: 5; --scaleY: -10; }
        .nf404 .code-message-text span:nth-of-type(4) { --scaleX: 40; --scaleY: -30; }
        .nf404 .code-message-text span:nth-of-type(6) { --scaleX: 50; --scaleY: 50; }
        .nf404 .code-message-text span:nth-of-type(7) { --scaleX: 0; --scaleY: -15; }
        .nf404 .code-message-text span:nth-of-type(8) { --scaleX: 35; --scaleY: 10; }
        .nf404 .code-message-text span:nth-of-type(9) { --scaleX: -40; --scaleY: 25; }
        .nf404 .code-message-text span:nth-of-type(10) { --scaleX: 50; --scaleY: -25; }

        .nf404 .bear-logo,
        .nf404 .code-text span:nth-of-type(2):after,
        .nf404 .code-text span:nth-of-type(3):after,
        .nf404 .code-text span:nth-of-type(4):after,
        .nf404 .code-message-text span:nth-of-type(2):after,
        .nf404 .code-message-text span:nth-of-type(3):after,
        .nf404 .code-message-text span:nth-of-type(4):after,
        .nf404 .code-message-text span:nth-of-type(6):after,
        .nf404 .code-message-text span:nth-of-type(7):after,
        .nf404 .code-message-text span:nth-of-type(8):after,
        .nf404 .code-message-text span:nth-of-type(9):after,
        .nf404 .code-message-text span:nth-of-type(10):after {
      ...
      <main className="app-shell">
        <div className="container-grid">
          <a className="return-link" href={homeHref}>
            {homeLabel}
          </a>
          <BearLogo className="bear-logo--tears" />
          <GlitchyText className="code-text">404</GlitchyText>
          <GlitchyText className="code-message-text">Not found</GlitchyText>
        </div>
      </main>
    </div>
  );
